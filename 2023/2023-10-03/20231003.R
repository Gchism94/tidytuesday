...
pacman::p_load(animation, # convert pdf to png
               cowplot, # Tools to add an image to a plot #<2>
...
p1 <- data |>
  ...
  labs(x = "Date", 
       ...
       caption = "Federal Food, Health and Social Services and Environmental Sustainability and Natural Resources grants<br>provide the largest amount through the most grants to Native American Government bodies.",
       color = NULL) + 
  scale_color_manual(
    values = pal,
...
p3 <- data %>%
  ...
  labs(caption = "Source: tidytuesday data | Viz: Greg Chism") +
  theme(
    ...
    plot.caption = element_markdown(size = 14, hjust = 0.5, color = "gray40")
  )
...
ggsave(plot = last_plot(), here("2023", "2023-10-03", "20231003.pdf"), width = 7000, height = 9000, units = "px", dpi = 500)
